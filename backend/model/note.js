...
function getNotesForOwner(id_owner, callback) {
    ...
    client.connect(function (err) {
        var notes = [];
        var query = client.query('select users.id as id, firstname, lastname, postal_code, town, date_start, date_end, capacity, attitude, degradation from t_notes as note inner join t_towns as town on town.id=note.id_town inner join t_users as users on users.id=note.id_tenant where id_owner=$1 order by note.id',
            [id_owner]);
        query.on('row', function (row) {
            notes.push(row);
        });
